...
from dataset.sp_esca_tissue_dataset import ESCATissueSuperpixelsDataset
...
dataset = ESCATissueSuperpixelsDataset(root='/mnt/s3/lhm/ESCA_crop/pre')
loader = DataLoader(dataset, batch_size=1, shuffle=True)
device = torch.device('cuda:1')
weight = tensor([1, 0.8]).to(device)
# criterion = nn.CrossEntropyLoss()
criterion = focal_loss
clsfr = Mlp(in_features=256, hidden_features=128, out_features=21).to(device)

graph_embedding = GraphNet(c_in=768, hidden_size=256, nc=21).to(device)
# encoder = Encoder().to(device)
encoder = GraphTransformer().to(device)
opt = torch.optim.SGD(
...
# graph_embedding.load_state_dict(torch.load('model_save/esca/graph_embedding.pth'))
# encoder.load_state_dict(torch.load('model_save/esca/encoder.pth'))
# clsfr.load_state_dict(torch.load('model_save/esca/clsfr.pth'))

print("model state dict Loaded")
train_acc = torchmetrics.Accuracy(task="multiclass", num_classes=21, average=None).to(device)
...
for epoch in range(1000):
    ...
    for index, batch in enumerate(loader):
        try:
            ...
            transformer 处理
            '''
            padding_vector = [False] * node_num + [True] * (2048 - node_num)
            # 将这个行向量复制seq_length行，形成所需的mask矩阵
            padding_mask = torch.from_numpy(np.tile(padding_vector, (2048, 1))).to(device)
    ...
    train_acc.reset()
    torch.save(graph_embedding.state_dict(), 'model_save/esca/graph_embedding.pth')
    torch.save(encoder.state_dict(), 'model_save/esca/encoder.pth')
    torch.save(clsfr.state_dict(), 'model_save/esca/clsfr.pth')
