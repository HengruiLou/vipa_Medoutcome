...
transformer1 = Transformer(embed_dim=256, cls=5).to(device)
...
final_clsfr2 = Mlp(in_features=512, hidden_features=196, out_features=5).to(device)
final_clsfr3 = Mlp(in_features=512, hidden_features=196, out_features=5).to(device)
...
graph_transformer_3 = GraphTransformer(cls=5).to(device)
...
train_acc1 = torchmetrics.Accuracy(task="multiclass", num_classes=5, average=None).to(device)
train_acc2 = torchmetrics.Accuracy(task="multiclass", num_classes=5, average=None).to(device)
train_acc3 = torchmetrics.Accuracy(task="multiclass", num_classes=5, average=None).to(device)
logging.basicConfig(
    level=logging.DEBUG,
...
for epoch in range(100):
    ...
    for img_idx, batch in enumerate(loader):
        ...
        transformer 处理
        '''
        x_class_all, cls_token, x_class_node, attn = transformer1(feat)
        ...
        g_attn1 = \
        torch.autograd.grad(outputs=nn.Softmax(dim=1)(x_class_all)[:, 1].sum(), inputs=attn, retain_graph=True)[0]
        g_attn1 = g_attn1.sum(dim=(1, 2))[:, 1:]
        g_attn1 = g_attn1.squeeze()
        loss = criterion(F.softmax(x_class_all, dim=1), y1) / 4
        loss.backward()
        if img_idx % 4 == 0:
            opt1.step()
            opt1.zero_grad()
        pred_y = x_class_all.argmax(dim=1)
        ...
        res = multi_process_exec(convert_numpy_img_to_superpixel_graph_2_single, list(
            zip(patches_2, repeat(y1.cpu().item()), centers_stage_2 - (1024 * 8 // 2), repeat(seg_map_stage_1),
                repeat(batch.offset), repeat(size))), 32)
